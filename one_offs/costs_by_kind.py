...
def taskgraph_cost_by_kinds(graph, worker_costs):
    ...
    for task in graph.tasks():
        ...
        if kind == '':
            continue
        total_wall_time_buckets[kind][worker] += sum(task.run_durations(), timedelta(0))

        if task.completed:
            final_task_wall_time_buckets[kind][worker] += task.resolved - task.started

    total_cost = 0.0
    final_task_costs = 0.0
    kind_df = pd.DataFrame(columns=['kind', 'cost', 'time'])
    ...
    for kind in total_wall_time_buckets:
        ...
        kind_df = kind_df.append({'kind': kind, 'cost': kind_cost, 'time': kind_hours}, ignore_index=True)

    print(kind_df)
    kind_df.to_csv('kinds_{}.csv'.format(graph.groupid))
